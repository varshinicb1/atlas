...
function findCli() {
    ...
    return (0, cliArgs_1.resolveCliPath)(cfg.get("cliPath"));
}
...
class AtlasExplorerPanel {
    ...
    constructor(panel, extensionUri) {
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                case "solve":
                    try {
                        const result = await runAtlas((0, cliArgs_1.buildSolveArgs)(msg.bundle, msg.query));
                        this._panel.webview.postMessage({
                            command: "solveResult",
                            data: (0, cliArgs_1.parseCliJson)(result),
                        });
                    }
                    catch (e) {
                        this._panel.webview.postMessage({
                ...
                case "dump":
                    try {
                        const result = await runAtlas((0, cliArgs_1.buildDumpArgs)(msg.bundle));
                        this._panel.webview.postMessage({
                            command: "dumpResult",
                            data: (0, cliArgs_1.parseCliJson)(result),
                        });
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel("atlasExplorer", "Atlas Knowledge Graph", column, { enableScripts: true, retainContextWhenHidden: true });
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("atlas.explore", () => {
        AtlasExplorerPanel.createOrShow(context.extensionUri);
    }));
    context.subscriptions.push(vscode.commands.registerCommand("atlas.search", async () => {
        ...
        if (!query)
            return;
        try {
            const result = await runAtlas((0, cliArgs_1.buildSearchArgs)(query));
            const data = (0, cliArgs_1.parseCliJson)(result);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("atlas.compile", async (uri) => {
        ...
        if (!(0, cliArgs_1.isCompilable)(filePath)) {
            vscode.window.showErrorMessage("Atlas compiles .md and .yaml files");
            return;
        }
        try {
            const outPath = (0, cliArgs_1.outputPathFor)(filePath);
            const result = await runAtlas((0, cliArgs_1.buildCompileArgs)(filePath, outPath));
            const data = (0, cliArgs_1.parseCliJson)(result);
            vscode.window.showInformationMessage(`Compiled ${data.nodes} nodes, ${data.edges} edges → ${data.output}`);
        }
        catch (e) {
            vscode.window.showErrorMessage(`Compile failed: ${errorMessage(e)}`);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand("atlas.solve", async () => {
        const query = await vscode.window.showInputBox({
        ...
        if (!query)
            return;
        try {
            const bundlePath = (0, cliArgs_1.resolveBundlePath)(vscode.workspace.getConfiguration("atlas").get("bundlePath"));
            const result = await runAtlas((0, cliArgs_1.buildSolveArgs)(bundlePath, query));
            const data = (0, cliArgs_1.parseCliJson)(result);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("atlas.decide", async () => {
        const query = await vscode.window.showInputBox({
        ...
        const ctxArgs = (0, cliArgs_1.parseContextArgs)(ctx);
        try {
            const bundlePath = (0, cliArgs_1.resolveBundlePath)(vscode.workspace.getConfiguration("atlas").get("bundlePath"));
            const result = await runAtlas((0, cliArgs_1.buildDecideArgs)(bundlePath, query, ctxArgs));
            if (result.trim() === "null") {
                vscode.window.showInformationMessage("No matching decision tree found.");
                return;
            }
            const data = (0, cliArgs_1.parseCliJson)(result);
            vscode.window.showInformationMessage(`Decision: ${data.rationale}\n\nPath: ${data.path.join(" → ")}`);
        }
        catch (e) {
            vscode.window.showErrorMessage(`Decide failed: ${errorMessage(e)}`);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand("atlas.init", async () => {
        ...
        if (!template)
            return;
        try {
            const result = await runAtlas((0, cliArgs_1.buildInitArgs)(name, template));
