...
server.registerTool("atlas_solve", {
  ...
  try {
    ...
    const lines = result.nodes.map(
      (n: atlas.NodeResult) =>
        `[${n.kind}] ${n.name} (${n.id})${n.version ? ` v${n.version}` : ""}${n.description ? `\n  ${n.description.slice(0, 200)}` : ""}`
    );
    return {
...
server.registerTool("atlas_decide", {
  ...
  try {
    const ctx = context_json ? JSON.parse(context_json) as Record<string, string> : undefined;
    ...
    const lines = [
      ...
      "",
      "Recommendations:",
      ...result.recommendations.map((r: { node_id: string; confidence: number }) => `  ${r.node_id} (confidence: ${r.confidence})`),
    ];
    if (result.agent_instructions) lines.push("", `Agent instructions: ${result.agent_instructions}`);
...
server.registerTool("atlas_verify", {
  ...
  try {
    ...
    const lines = [
      ...
      "",
      ...result.checks.map((c: { name: string; passed: boolean; message: string }) => `${c.passed ? "✓" : "✗"} ${c.name}: ${c.message}`),
    ];
    return { content: [{ type: "text", text: lines.join("\n") }] };
  } catch (e: unknown) {
    return { content: [{ type: "text", text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
...
server.registerTool("atlas_reason", {
...
server.registerTool("atlas_navigate", {
  ...
  try {
    ...
    const node = ir.nodes.find((n: atlas.NodeResult) => n.id === node_id);
    if (!node) {
      return { content: [{ type: "text", text: `Node "${node_id}" not found.` }] };
    }
    const outgoing = ir.edges.filter((e: { src: string }) => e.src === node_id);
    const incoming = ir.edges.filter((e: { dst: string }) => e.dst === node_id);
    const neighborIds = new Set<string>();
    outgoing.forEach((e: { dst: string }) => neighborIds.add(e.dst));
    incoming.forEach((e: { src: string }) => neighborIds.add(e.src));
    const neighbors = ir.nodes.filter((n: atlas.NodeResult) => neighborIds.has(n.id));
    const lines = [
      ...
      ...outgoing.map((e: { dst: string; edge_type: string }) => {
        const t = ir.nodes.find((n: atlas.NodeResult) => n.id === e.dst);
        return `  → ${e.edge_type} → ${t ? `[${t.kind}] ${t.name}` : e.dst}`;
      }),
      "",
      `Incoming edges (${incoming.length}):`,
      ...incoming.map((e: { src: string; edge_type: string }) => {
        const s = ir.nodes.find((n: atlas.NodeResult) => n.id === e.src);
        return `  ← ${e.edge_type} ← ${s ? `[${s.kind}] ${s.name}` : e.src}`;
      }),
      "",
      `Connected nodes (${neighbors.length}):`,
      ...neighbors.map((n: atlas.NodeResult) => `  [${n.kind}] ${n.name} (${n.id})`),
    ];
    return { content: [{ type: "text", text: lines.join("\n") }] };
  } catch (e: unknown) {
    return { content: [{ type: "text", text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
...
server.registerResource("bundle", new ResourceTemplate("atlas://bundle/{path}", { list: undefined }), {
